...
std::string& GeoLite2PP::DB::get_field(MMDB_lookup_result_s *lookup, const char** v)
{
  if (lookup) {
    ...
    if (result.has_data) {
      switch (result.type) {
      case MMDB_DATA_TYPE_UTF8_STRING:  str_ = std::string(result.utf8_string, result.data_size );	break;
      case MMDB_DATA_TYPE_DOUBLE:       str_ = std::to_string(result.double_value               );	break;
      case MMDB_DATA_TYPE_UINT16:       str_ = std::to_string(result.uint16                     );	break;
      case MMDB_DATA_TYPE_UINT32:       str_ = std::to_string(result.uint32                     );	break;
      case MMDB_DATA_TYPE_INT32:        str_ = std::to_string(result.uint32                     );	break;
      case MMDB_DATA_TYPE_UINT64:       str_ = std::to_string(result.uint64                     );	break;
      case MMDB_DATA_TYPE_FLOAT:        str_ = std::to_string(result.float_value                );	break;
      case MMDB_DATA_TYPE_BOOLEAN:      str_ = (result.boolean ? "true" : "false"               );	break;
